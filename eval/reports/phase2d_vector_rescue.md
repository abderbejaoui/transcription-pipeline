# Correction Pipeline Evaluation — `phase2d_vector_rescue`

**Date:** 2026-06-01 20:43:43  
**Elapsed:** 53.3s  
...
| **Mean WER (corrected → gold)** | 0.2321 |
| **WER reduction (Δ)** | **+0.0984** |
| **Correction precision** | 0.0778 (36/463) |
| **Correction recall** | 0.4056 (58/143) |
| **F1 score** | 0.1305 |
| **Do-no-harm rate** | 0.8812 (89/101) |
| **Total flags (HITL)** | 143 |
| **Avg flags/record** | 0.66 |
| **Avg corrections applied/record** | 2.14 |
...
| en | 97 | 0.1406 | 0.1130 | +0.0276 |
| mixed | 15 | 0.2390 | 0.1681 | +0.0710 |

### WER by Difficulty

| Difficulty | N | WER (raw) | WER (corrected) | Δ |
|------------|---|-----------|-----------------|----|
| easy | 122 | 0.1622 | 0.1020 | +0.0601 |
| hard | 33 | 0.5073 | 0.3496 | +0.1577 |
| medium | 61 | 0.5713 | 0.4285 | +0.1428 |
...
| eval_0014 | en | hard | Yes | 0.029 | 0.118 | -0.088 | 5 | 3 | ✅ |
| eval_0015 | en | hard | Yes | 0.094 | 0.156 | -0.062 | 2 | 2 | ✅ |
| eval_0016 | en | hard | Yes | 0.088 | 0.029 | +0.059 | 15 | 4 | ✅ |
| eval_0017 | en | medium | No | 0.000 | 0.100 | -0.100 | 21 | 2 | ❌ |
| eval_0018 | en | medium | No | 0.000 | 0.097 | -0.097 | 27 | 3 | ❌ |
| eval_0019 | en | hard | No | 0.000 | 0.000 | +0.000 | 0 | 0 | ✅ |
| eval_0020 | en | hard | No | 0.000 | 0.069 | -0.069 | 21 | 1 | ❌ |
| eval_0021 | en | hard | No | 0.000 | 0.152 | -0.152 | 21 | 1 | ❌ |
...
| eval_0030 | en | medium | Yes | 0.250 | 0.750 | -0.500 | 3 | 2 | ✅ |
...
| eval_0091 | en | easy | No | 0.000 | 0.625 | -0.625 | 3 | 1 | ❌ |
...
| eval_0105 | en | easy | No | 0.000 | 0.200 | -0.200 | 3 | 1 | ❌ |
| eval_0106 | en | easy | No | 0.000 | 0.250 | -0.250 | 3 | 1 | ❌ |
...
| eval_0114 | en | easy | No | 0.000 | 0.200 | -0.200 | 1 | 1 | ❌ |
...
| eval_0128 | en | easy | No | 0.000 | 0.125 | -0.125 | 6 | 1 | ❌ |
...
| eval_0193 | mixed | hard | Yes | 0.429 | 0.286 | +0.143 | 5 | 2 | ✅ |
| eval_0194 | en | medium | Yes | 0.167 | 0.500 | -0.333 | 3 | 2 | ✅ |
| eval_0195 | en | easy | Yes | 0.250 | 0.500 | -0.250 | 2 | 2 | ✅ |
| eval_0196 | en | medium | Yes | 0.200 | 0.600 | -0.400 | 2 | 1 | ✅ |
...
| eval_0212 | en | medium | Yes | 0.200 | 0.200 | +0.000 | 1 | 2 | ✅ |
...
**12 false positives (changes to clean input):**

- **eval_0021**: 'vasculitis,'→'vasculitis', 'and'→'prednisone', 'we'→'if', 'may'→'the', 'use'→'renal', 'prednisone'→'studies', 'if'→'worsen.', 'the'→'For', 'renal'→'now,', 'studies'→'continue', 'worsen.'→'close', 'For'→'follow-up', 'now,'→'and', 'continue'→'return', 'close'→'if', 'follow-up'→'you', 'and'→'notice', 'return'→'blood', 'if'→'in', 'you'→'the', 'notice'→'urine.'
- **eval_0106**: 'a'→'history', 'history'→'well-controlled', 'of'→'hypertension.'
- **eval_0099**: 'examination'→'reveals', 'reveals'→'no', 'no'→'abnormalities.'
- **eval_0176**: 'المفاصل'→'myofascial'
- **eval_0128**: 'new'→'symptoms', 'symptoms'→'were', 'were'→'reported', 'reported'→'at', 'at'→'today's', 'today's'→'visit.'
- **eval_0105**: 'pain'→'shortness', 'or'→'of', 'shortness'→'breath.'
- **eval_0018**: 'low'→'blood', 'blood'→'pressure', 'pressure'→'in', 'in'→'septic', 'septic'→'shock,', 'shock,'→'we', 'we'→'are', 'are'→'continuing', 'continuing'→'norepinephrine', 'norepinephrine'→'vasopressin', 'and'→'while', 'vasopressin'→'watching', 'while'→'your', 'watching'→'urine', 'your'→'output', 'urine'→'and', 'output'→'lactate', 'and'→'closely.', 'lactate'→'The', 'closely.'→'infectious', 'The'→'doses', 'infectious'→'team', 'disease'→'agrees', 'team'→'with', 'agrees'→'vancomycin', 'with'→'for', 'vancomycin'→'now.'
- **eval_0017**: 'the'→'thrombocytopenia', 'thrombocytopenia'→'will', 'will'→'still', 'still'→'need', 'need'→'repeat', 'repeat'→'monitoring.', 'monitoring.'→'We', 'We'→'are', 'are'→'not', 'not'→'changing', 'changing'→'the', 'the'→'methylprednisolone,', 'methylprednisolone'→'and', 'dose,'→'the', 'and'→'nephrology', 'the'→'team', 'nephrology'→'still', 'team'→'believes', 'still'→'this', 'believes'→'is', 'this'→'glomerulonephritis.'
- **eval_0147**: 'نظيف'→'نزيف'
- **eval_0020**: 'leukemia,'→'leukemia', 'and'→'hematology', 'hematology'→'has', 'has'→'already', 'already'→'started', 'started'→'all-trans', 'all-trans'→'retinoic', 'retinoic'→'acid.', 'acid.'→'Because', 'Because'→'of', 'of'→'the', 'the'→'coagulopathy', 'coagulopathy'→'risk,', 'risk,'→'the', 'the'→'patient', 'patient'→'will', 'will'→'remain', 'remain'→'in', 'in'→'a', 'a'→'monitored', 'monitored'→'setting.'
- **eval_0091**: '120/80'→'temp', 'HR'→'RR', '72'→'16'
- **eval_0114**: 'is'→'unremarkable.'

**80 records with missed corrections:**

- **eval_0030** (lang=en, diff=medium)
  - Gold: 'clopidogr 75 mg daily'→'clopidogrel 75 mg daily'
  - Changes applied: 'clopidogr'→'clopidogral', '75'→'myasthenia', 'mg'→'gravis'
