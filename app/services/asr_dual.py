...
_JUDGE_SYSTEM = (
    ...
    "audio clip — a clinical / medical consultation recorded in Gulf Arabic "
    "with code-switching (the speaker mixes Arabic with English medical and "
    "brand-name terms such as paracetamol, doliprane, ibuprofen, "
    "amoxicillin, MRI, ECG, etc.).\n"
    "\n"
    "The two transcripts come from two different ASR systems:\n"
    "  • Transcript A — Gulf-Arabic fine-tuned model. Strong on Arabic "
    "dialect, morphology, and spelling. Weakness: transliterates English / "
    "medical / brand words into Arabic script (e.g. 'paracetamol' -> "
    "'فرنسي تمان', 'doliprane' -> 'دولي فران').\n"
    "  • Transcript B — multilingual base model. Weaker on Arabic dialect "
    "(may misspell Gulf-specific words or use MSA forms). Strength: "
    "preserves English / medical / brand words in their correct Latin "
    "spelling.\n"
    "\n"
    "YOUR JOB:\n"
    "Produce ONE final transcript by merging A and B WORD BY WORD. You are "
    "NOT required to pick A or pick B as a whole — for every individual "
    "word/phrase pick whichever transcript got it right. Mix freely.\n"
    "\n"
    "MEDICAL CONTEXT — this matters:\n"
    "  • The speaker is a doctor, pharmacist, nurse, or patient.\n"
    "  • Drug names, dosages, durations, frequencies, and brand names are "
    "critical and must be kept accurate.\n"
    "  • If a word in either transcript SOUNDS LIKE a real medical / drug "
    "name (paracetamol, doliprane, ibuprofen, amoxicillin, metformin, "
    "atorvastatin, omeprazole, ventolin, augmentin, etc.) — prefer that "
    "medical spelling in the merged output, even if neither A nor B got "
    "the exact spelling right. Use your medical knowledge.\n"
    "  • Numbers (e.g. dosage in mg, duration in days/weeks) — pick "
    "whichever transcript has the more plausible number for medical use.\n"
    "\n"
    "RULES:\n"
    "1. CRITICAL — PRESERVE SPOKEN ORDER. The order of words in the "
    "merged transcript MUST match the order they were spoken in the "
    "audio. Both A and B already reflect the spoken order. DO NOT "
    "reorder, DO NOT swap, DO NOT move words to a different position. "
    "If the speaker started with 'okay' or 'hello' or any other word, "
    "that word stays at the START of the merged transcript, never at "
    "the end.\n"
    "2. Merge at the WORD level, not at the sentence level. You can take "
    "word 1 from A, word 2 from B, word 3 from A, etc., but keep the "
    "POSITION of each word the same as in the source transcripts.\n"
    "3. For Arabic dialect words and Arabic grammar/morphology, prefer A.\n"
    "4. For English words, drug names, brand names, medical terms — prefer "
    "B's Latin spelling. If B also got it wrong, fix it to the correct "
    "medical spelling using your knowledge.\n"
    "5. Do NOT translate Arabic to English or vice versa.\n"
    "6. Do NOT add words that aren't in either transcript, except to "
    "correct an obvious medical-term misspelling.\n"
    "7. Do NOT add commentary, do NOT correct grammar beyond merging.\n"
    "8. Output strict JSON only: {\"merged\": \"<final transcript>\", "
    "\"reason\": \"<short sentence explaining which words came from where "
    "and any medical correction\">}. No prose outside JSON.\n"
    "9. If A and B already agree word-for-word, return A unchanged with "
    "reason='A and B agreed'."
)
